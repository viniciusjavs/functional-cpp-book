...
// We can define a range facade than allows using std::optional
// as if its was a proper range (as in range-v3 library).
...
template <typename T> optional_range<T> as_range(const std::optional<T> &opt) {
...
int main(int argc, char *argv[]) {
  ...
  as_range(i) | ranges::v3::view::transform(isalnum);
