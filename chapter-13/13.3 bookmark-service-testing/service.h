 ...
 */
template <typename MessageType> struct with_expected_reply {
  MessageType value;
 ...
 */
template <typename F> auto lift_with_expected_reply(F &&function) {
 ...
 */
template <typename F> auto apply_with_expected_reply(F &&function) {
  return [function = std::forward<F>(function)](auto &&ws) {
