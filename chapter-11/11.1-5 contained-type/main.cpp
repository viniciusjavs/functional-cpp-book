...
// contained_type<C>
template <typename C> auto sum(const C &collection) {
...
int main(int argc, char *argv[]) {
  ...
  static_assert(
      is_same<contained_type<std::vector<std::string>>, std::string>(),
  ...
  std::cout << sum_iterable(xs) << '\n';
  std::cout << sum_collection(xs) << '\n';
  std::cout << sum(xs) << '\n';
