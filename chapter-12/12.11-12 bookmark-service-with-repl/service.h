 ...
 */
template <typename MessageType> struct with_client {
  MessageType value;
  tcp::socket *socket;
 ...
 */
template <typename F> auto lift_with_client(F &&function) {
 ...
 * with socket pointer
 */
template <typename F> auto apply_with_client(F &&function) {
  return [function = std::forward<F>(function)](auto &&ws) {
 ...
 * be used for sending the messages
 */
...
public:
  ...
  template <typename EmitFunction> void on_message(EmitFunction emit) {
    m_emit = emit;
    do_accept();
  }
