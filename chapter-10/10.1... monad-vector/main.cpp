...
template <typename T> std::vector<T> make_vector(T &&value) {
  return {std::forward<T>(value)};
}

template <typename T, typename F> auto mbind(const std::vector<T> &xs, F f) {
  ...
  // This means that mbind also return std::vector<R>
  using result_vector_type = std::invoke_result_t<F, T>;

  // invoke_result_t can be replace with decltype(f(xs[0]))
  ...
  for (const auto &x : xs) {
    ...
    result.insert(result.end(), std::make_move_iterator(transformed.begin()),
...
namespace detail {
template <typename F> struct mbind_helper { F f; };
} // namespace detail

template <typename F> detail::mbind_helper<F> mbind(F &&f) {
  return {std::forward<F>(f)};
}

template <typename T, typename F>
auto operator|(const std::vector<T> &xs,
               const detail::mbind_helper<F> &helper) {
...
int main(int argc, char *argv[]) {
  ...
  auto files_in_dir = [](const std::string &dir) {
    return std::vector<std::string>{dir + "/file1.txt", dir + "/file2.txt"};
  };
